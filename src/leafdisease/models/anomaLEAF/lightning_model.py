...
class anomaLEAF(pl.LightningModule):
    ...
    def training_step(
        ...
        # create masks
        input, _, _ = self.model.mask_input(padded)

        ##########################
        # Optimize Discriminator #
        ##########################
        fake = self.model.generator(input)
        pred_real, _ = self.model.discriminator(input_tensor=input, target_tensor=padded)
        pred_fake, _ = self.model.discriminator(input_tensor=input, target_tensor=fake)
        ...
        pred_fake, _ = self.model.discriminator(input_tensor=padded, target_tensor=fake)
    ...
    def validation_step(self, batch: dict[str, str | Tensor], *args, **kwargs) :
        ...
        pred_real = self.model.discriminator(input_tensor=input, target_tensor=padded)
        pred_fake = self.model.discriminator(input_tensor=input, target_tensor=fake)
    ...
    def predict_step(self, batch: dict[str, str | Tensor], batch_idx):
        ...
        padded, fake = self(batch["image"])

        return {"real": padded, "generated": fake, "filename": batch["filename"]}

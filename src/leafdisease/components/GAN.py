...
class Discriminator(nn.Module):
    ...
    def __init__(
        ...
        depth = math.ceil(math.log(min(input_size)//2,2)) - 2
...
class Generator(nn.Module):
    ...
    def __init__(
        ...
        assert depth < math.log(min(input_size),2), f"Encoding {depth} too deep for inputs size {input_size}. Max depth {math.log(min(input_size),2)}"
...
class Decoder(nn.Module):
    ...
    def __init__(
        ...
        n_input_features = n_features * (2**depth)
